...
class VAE(EstimatorTransformer, DLEstimatorMixin):
    ...
    def __init__(self, encoder: "torch.nn.Module", decoder: "torch.nn.Module", beta: float = 1.0,
        ...
        # NEW: Store early stopping parameters
    ...
    def evaluate_loss(self, x: "torch.Tensor"):
        # ... (no changes to this method, but removed the print statement for clarity)
        mu, logvar = self._encoder(x)
        z = _reparameterize(mu, logvar)
        x_hat = self._decoder(z)
        reconstruction_loss = self._mse_loss(x_hat, x)
        kld_loss = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
        return reconstruction_loss + self._beta * kld_loss
    ...
    def fit(self, data_loader: "torch.utils.data.DataLoader", n_epochs: int = 100,
        ...
        for epoch in range(n_epochs):
            ...
            for batch in data_loader:
                ...
                loss_value = self.evaluate_loss(batch)
                loss_value.backward()
                self.optimizer.step()
            ...
            if validation_loader is not None:
                ...
                with torch.no_grad():
                    ...
                    for batch in validation_loader:
                        batch = batch.to(device=self.device)
                        loss_value = self.evaluate_loss(batch).item()
        ...
        # NEW: Restore the best model weights after training is finished (or stopped)
